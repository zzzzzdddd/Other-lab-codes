...
def load_mnist_trainval():
    ...
    train_data = data[0:int(0.8*len(label))]
    train_label = label[0:int(0.8*len(label))]
    val_data = data[int(0.8*len(label)):]
...
def generate_batched_data(data, label, batch_size=32, shuffle=False, seed=None):
    ...
    # if shuffle:
    #     random.shuffle(inds)
    # shuffled_data = [data[i] for i in inds]
    # shuffled_label = [label[i] for i in inds]
    batched_data = []
    batched_label = []
    # pt = 0
    # while pt<len(label):
    #     last_size = len(label) - pt + 1
    #     if last_size >= batch_size:
    #         ea = np.zeros((batch_size,784))
    #         eb = np.zeros((batch_size,1))
    #         for i in range(batch_size):
    #             ea[i] = shuffled_data[pt]
    #             eb[i] = shuffled_label[pt]
    #             pt += 1
    #     else:
    #         ea = np.zeros((last_size,784))
    #         eb = np.zeros((last_size,1))
    #         for i in range(last_size):
    #             ea[i] = shuffled_data[pt]
    #             eb[i] = shuffled_label[pt]
    #             pt += 1
    #     batched_data.append(ea)
    #     batched_label.append(eb)
    data = np.array(data)
    label = np.array(label)
    ind = np.arange(len(label))
    nb = int(np.ceil(len(label)/batch_size))
    if shuffle:
        random.shuffle(ind)
    data = data[ind]
    label = label[ind]
    for i in range(nb):
        ...
        batched_label.append(label[batch_size*i:batch_size*(i+1)])
    #############################################################################
    #                              END OF YOUR CODE                             #
    #############################################################################
...
def train(epoch, batched_train_data, batched_train_label, model, optimizer, debug=False):
    """
...
def plot_curves(train_loss_history, train_acc_history, valid_loss_history, valid_acc_history):
    ...
    axs[0].plot(x, train_loss_history, label = 'train')
    axs[0].plot(x, valid_loss_history, label = 'valid')
    axs[0].set(xlabel='epoch', ylabel='loss',title='loss curve')
    axs[0].legend(loc = 'upper right')
    axs[1].plot(x, train_acc_history, label = 'train')
    axs[1].plot(x, valid_acc_history, label = 'valid')
    axs[1].set(xlabel='epoch', ylabel='accuracy',title='accuracy curve')
