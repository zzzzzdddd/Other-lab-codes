...
class TwoLayerNet(_baseNetwork):
    ...
    def forward(self, X, y, mode='train'):
        ...
        #       outputs                                                             #
        #############################################################################
        l1 = X@self.weights['W1'] + self.weights['b1']
        l1a = self.sigmoid(l1)
        l2 = l1a@self.weights['W2'] + self.weights['b2']
        out = self.softmax(l2)
        ...
        #############################################################################
        gr2 = out.copy()
        gr2[range(len(y)), y.astype(int)] -= 1
        gr2 /= len(y)
        self.gradients['W2'] = l1a.T@gr2
        self.gradients['b2'] = sum(gr2)
        dl2 = gr2@self.weights['W2'].T
        gr1 = l1a * (1 - l1a)
        gr = dl2 * gr1
        self.gradients['W1'] = X.T@gr
        self.gradients['b1'] = sum(gr)
        #############################################################################
        #                              END OF YOUR CODE                             #
        #############################################################################
