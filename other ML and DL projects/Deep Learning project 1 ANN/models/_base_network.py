...
class _baseNetwork:
    ...
    def softmax(self, scores):
        ...
        for i in range(len(scores)):
            sig = np.sum(np.exp(scores[i]-np.max(scores[i])))
            es = np.exp(scores[i]-(np.max(scores[i])))
            prob[i] = es/sig
        #############################################################################
        #                              END OF YOUR CODE                             #
        #############################################################################
    ...
    def compute_accuracy(self, x_pred, y):
        ...
        pre = y.copy()
        for i in range(len(y)):
            pre[i] = np.argmax(x_pred[i])
        acc = sum(np.equal(pre, y)*1)/len(y)
        #############################################################################
        #                              END OF YOUR CODE                             #
        #############################################################################
        return acc
