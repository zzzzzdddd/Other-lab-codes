...
class VanillaRNN(nn.Module):
    ...
    def __init__(self, input_size, hidden_size, output_size):
        ...
        self.hiddenu = nn.Linear(self.input_size + self.hidden_size, self.hidden_size)
        # initialize output unit
        self.outputu = nn.Linear(self.input_size + self.hidden_size, self.output_size)
    ...
    def forward(self, input, hidden):
        ...
        #############################################################################
        output = self.LS(self.outputu(torch.cat((input, hidden), dim = 1)))
        hidden = self.tanh(self.hiddenu(torch.cat((input, hidden), dim = 1)))
        #############################################################################
        #                              END OF YOUR CODE                             #
        #############################################################################
