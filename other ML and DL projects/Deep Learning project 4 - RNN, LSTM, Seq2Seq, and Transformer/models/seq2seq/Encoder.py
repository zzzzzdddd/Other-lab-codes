...
class Encoder(nn.Module):
    ...
    def __init__(self, input_size, emb_size, encoder_hidden_size, decoder_hidden_size, dropout=0.2, model_type="RNN"):
        ...
        if self.model_type=="RNN":
            self.rec = nn.RNN(self.emb_size,self.encoder_hidden_size, batch_first=True)
        elif self.model_type=="LSTM":
            self.rec = nn.LSTM(self.emb_size,self.encoder_hidden_size, batch_first=True)
        self.l1 = nn.Linear(self.encoder_hidden_size, self.encoder_hidden_size)
        self.relu = nn.ReLU()
        self.l2 = nn.Linear(self.encoder_hidden_size, self.encoder_hidden_size)
    ...
    def forward(self, input):
        ...
        tanh = nn.Tanh()
        dropemb = self.dp(self.emb(input))
        if self.model_type=="RNN":
            output, hidden = self.rec(dropemb)
            hidden = tanh(self.l2(self.relu(self.l1(hidden))))
        elif self.model_type=="LSTM":
            output, (hidden, c_n) = self.rec(dropemb)
            hidden = tanh(self.l2(self.relu(self.l1(hidden))))
            hidden = (hidden, c_n)
        #############################################################################
        #                              END OF YOUR CODE                             #
