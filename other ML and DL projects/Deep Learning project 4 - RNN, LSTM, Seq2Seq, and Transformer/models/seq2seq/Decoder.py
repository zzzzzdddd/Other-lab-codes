...
class Decoder(nn.Module):
    ...
    def __init__(self, emb_size, encoder_hidden_size, decoder_hidden_size, output_size, dropout=0.2, model_type="RNN"):
        ...
        if self.model_type=="RNN":
            self.rec = nn.RNN(self.emb_size, self.decoder_hidden_size, batch_first=True)
        elif self.model_type=="LSTM":
            self.rec = nn.LSTM(self.emb_size, self.decoder_hidden_size, batch_first=True)
    ...
    def forward(self, input, hidden):
        ...
        output, hidden = self.rec(dropemb, hidden)
        output = self.LS(self.l1(output.squeeze()))
        #############################################################################
        #                              END OF YOUR CODE                             #
        #############################################################################
