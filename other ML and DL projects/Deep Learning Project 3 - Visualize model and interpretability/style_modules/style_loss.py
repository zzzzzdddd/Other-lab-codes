...
class StyleLoss(nn.Module):
    def gram_matrix(self, features, normalize=True):
        ...
        feats1 = features.view(N, C, -1)
        # print(torch.transpose(features, 1, 2).size())
        feats2 = torch.transpose(feats1, 1, 2)
        g = torch.bmm(feats1, feats2)
        if normalize:
            g /= (H * W * C)
        return g
        ##############################################################################
    ...
    def forward(self, feats, style_layers, style_targets, style_weights):
        ...
        ##############################################################################
        # print(feats)
        style_loss = torch.tensor(0).float()
        for i in range(len(style_layers)):
            style_loss += style_weights[i] * \
                          torch.sum(torch.square(self.gram_matrix(feats[style_layers[i]]) - style_targets[i]))
        return style_loss
        ##############################################################################
        #                             END OF YOUR CODE                               #
        ##############################################################################
