...
class CustomReLU(TorchFunc):
    ...
    def backward(self, dout):
        ...
        ##############################################################################
        inds = (dout>0)*(self.saved_tensors[0]>0)*1
        # print(inds.float())
        dout = torch.addcmul(torch.zeros(dout.size()), inds.float(), dout)
        return dout
        ##############################################################################
        #                             END OF YOUR CODE                               #
        ##############################################################################
...
class GradCam:
    ...
    def guided_backprop(X_tensor, y_tensor, gc_model):
        ...
        W = X_tensor.shape[3]
        s = gc_model(X_tensor)
        ms = torch.gather(s, 1, y_tensor.view(-1, 1)).sum()
        ms.backward()
        gb = X_tensor.grad.view(N, H, W, 3)
        gb = gb.detach().numpy()
        return gb
        ##############################################################################
        #                             END OF YOUR CODE                               #
    ...
    def grad_cam(self, X_tensor, y_tensor, gc_model):
        ...
        ms = torch.gather(s, 1, y_tensor.view(-1, 1)).sum()
        ms.backward()
        gm = torch.mean(self.gradient_value, [2, 3])
        gm = gm.view(gm.shape[0], -1, 1, 1)
        self.activation_value *= gm
        cam = torch.mean(self.activation_value, 1).detach().numpy()
